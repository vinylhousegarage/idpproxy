package apierror

import (
	"errors"
	"net/http"
	"reflect"
	"testing"
)

func TestAPIErrors(t *testing.T) {
	originalErr := errors.New("base error")

	mockInternalErr := errors.New("internal debug info")

	tests := []struct {
		name           string
		fn             func(error, ...APIInternal) *APIError
		expectedCode   ErrorCode
		expectedStatus int
		internalsInfo  []APIInternal
	}{
		{
			name:           "MissingGitHubCode",
			fn:             MissingGitHubCode,
			expectedCode:   ErrorCodeMissingGitHubCode,
			expectedStatus: http.StatusBadRequest,
		},
		{
			name:           "MissingState",
			fn:             MissingState,
			expectedCode:   ErrorCodeMissingState,
			expectedStatus: http.StatusBadRequest,
		},
		{
			name:           "InvalidState",
			fn:             InvalidState,
			expectedCode:   ErrorCodeInvalidState,
			expectedStatus: http.StatusBadRequest,
			internalsInfo: []APIInternal{
				{Code: ErrorCodeInvalidQueryState, Err: mockInternalErr},
				{Code: ErrorCodeInvalidCookieState, Err: mockInternalErr},
			},
		},
		{
			name:           "GitHubAccessTokenRequestError",
			fn:             GitHubAccessTokenRequestError,
			expectedCode:   ErrorCodeGitHubAccessTokenRequest,
			expectedStatus: http.StatusBadGateway,
		},
		{
			name:           "GitHubTokenRequestError",
			fn:             GitHubTokenRequestError,
			expectedCode:   ErrorCodeGitHubTokenRequest,
			expectedStatus: http.StatusBadGateway,
		},
		{
			name:           "GitHubTokenExchangeError",
			fn:             GitHubTokenExchangeError,
			expectedCode:   ErrorCodeGitHubTokenExchange,
			expectedStatus: http.StatusBadGateway,
		},
		{
			name:           "GitHubUserRequestBuildError",
			fn:             GitHubUserRequestBuildError,
			expectedCode:   ErrorCodeGitHubUserRequestBuild,
			expectedStatus: http.StatusInternalServerError,
		},
		{
			name:           "GitHubUserRequestError",
			fn:             GitHubUserRequestError,
			expectedCode:   ErrorCodeGitHubUserRequest,
			expectedStatus: http.StatusBadGateway,
		},
		{
			name:           "GitHubUserDecodeError",
			fn:             GitHubUserDecodeError,
			expectedCode:   ErrorCodeGitHubUserDecode,
			expectedStatus: http.StatusBadGateway,
		},
		{
			name:           "GitHubUserUpsertError",
			fn:             GitHubUserUpsertError,
			expectedCode:   ErrorCodeGitHubUserUpsert,
			expectedStatus: http.StatusInternalServerError,
		},
		{
			name:           "InternalServerError",
			fn:             InternalServerError,
			expectedCode:   ErrorCodeInternalServerError,
			expectedStatus: http.StatusInternalServerError,
		},
		{
			name:           "ProxyCodeIssue",
			fn:             ProxyCodeIssue,
			expectedCode:   ErrorCodeProxyCodeIssue,
			expectedStatus: http.StatusInternalServerError,
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := tt.fn(originalErr, tt.internalsInfo...)

			if got.Code != tt.expectedCode {
				t.Errorf("expected code %s, got %s", tt.expectedCode, got.Code)
			}
			if got.HTTPStatus != tt.expectedStatus {
				t.Errorf("expected status %d, got %d", tt.expectedStatus, got.HTTPStatus)
			}
			if !errors.Is(got.Err, originalErr) {
				t.Error("expected original error to be wrapped")
			}
			if !reflect.DeepEqual(got.Internals, tt.internalsInfo) {
				t.Errorf("expected internal %v, got %v", tt.internalsInfo, got.Internals)
			}
		})
	}
}
